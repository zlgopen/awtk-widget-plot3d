...
class Plot3dTypeCylinderTest : public testing::Test {
 protected:
  void SetUp() {
    plot3d_unregister();
...
TEST_F(Plot3dTypeCylinderTest, cylinder_registered_in_register_all) {
  plot3d_type_t* type = plot3d_type_factory_create("cylinder");
  ASSERT_NE((plot3d_type_t*)NULL, type);
  ASSERT_EQ(RET_OK, plot3d_type_destroy(type));
}

TEST_F(Plot3dTypeCylinderTest, cylinder_layout_grid_outputs_flat_points) {
...
TEST_F(Plot3dTypeCylinderTest, cylinder_build_creates_cylinder_primitives) {
  plot3d_type_t* type = plot3d_type_factory_create("cylinder");
  plot3d_paint_ctx_t ctx;
  darray_t primitives;
  ...
  ASSERT_EQ(PLOT3D_PRIMITIVE_CYLINDER, primitive->type);
