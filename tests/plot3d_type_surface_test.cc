...
class Plot3dTypeSurfaceTest : public testing::Test {
 protected:
  void SetUp() {
    plot3d_unregister();
...
TEST_F(Plot3dTypeSurfaceTest, surface_registered_in_register_all) {
  plot3d_type_t* type = plot3d_type_factory_create("surface");
  ASSERT_NE((plot3d_type_t*)NULL, type);
  ASSERT_EQ(RET_OK, plot3d_type_destroy(type));
}

TEST_F(Plot3dTypeSurfaceTest, surface_layout_grid_triangulates_and_keeps_node_index) {
  plot3d_source_result_t result;
  plot3d_type_t* type = plot3d_type_factory_create("surface");
  plot3d_paint_ctx_t ctx;
  darray_t points;
  plot3d_data_point_t* p0 = NULL;
  plot3d_data_point_t* p3 = NULL;
  ...
  p0 = (plot3d_data_point_t*)darray_get(&points, 0);
  p3 = (plot3d_data_point_t*)darray_get(&points, 3);
  ASSERT_NE((plot3d_data_point_t*)NULL, p0);
  ASSERT_NE((plot3d_data_point_t*)NULL, p3);
  ASSERT_EQ(0u, p0->node_index);
  ASSERT_EQ(0u, p3->node_index);

  darray_deinit(&points);
  ASSERT_EQ(RET_OK, plot3d_type_destroy(type));
}

TEST_F(Plot3dTypeSurfaceTest, surface_build_creates_triangle_primitive) {
  plot3d_type_t* type = plot3d_type_factory_create("surface");
  plot3d_paint_ctx_t ctx;
  darray_t primitives;
  ...
  ASSERT_EQ(PLOT3D_PRIMITIVE_TRIANGLE, primitive->type);
  ASSERT_EQ(0u, primitive->i0);
  ASSERT_EQ(1u, primitive->i1);
  ASSERT_EQ(2u, primitive->i2);
